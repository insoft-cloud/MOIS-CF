...
export class UsermgmtService {
  ...
  userInfoAll(url, authorization) {
    return this.common.doGet(url+'/commonapi/v2/users','').map((res: any) => {
      return res;
    });
  }

  userInfo(username: string, url, authorization) {
    return this.common.doGet(url+'/commonapi/v2/user/' + username,'').map((res: any) => {
      return res;
    });
  }
  ...
  userAllDelete(guid: string, param:any){
    return this.common.doDelete('/commonapi/v2/user/' + guid+ '/all', '', this.common.getToken()).map((res: any) => {
      return res;
    });
  }
