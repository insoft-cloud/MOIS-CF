...
export class UsermgmtComponent implements OnInit {
  ...
  constructor(private httpClient: HttpClient, private common: CommonService, private userMgmtService: UsermgmtService, private translate: TranslateService,
    ...
    this.common.doPost('/portalapi/v3/user/cfpw',param,null).subscribe(data => {
      this.cfpw = data['Password'];
    }, error => {
  ...
  userInfo() {
    this.userMgmtService.userinfo(this.common.getUserGuid()).subscribe(data => {
      this.user = data;
      try {
  ...
  userSave() {
    ...
    this.userMgmtService.userSave(this.common.getUserGuid(), params).subscribe(data => {
      if (data === 1) {
  ...
  //           this.externalService.reset(param).subscribe(region => {
  //             forEachCount++;
  //             this.common.isLoading = false;
  ...
  get email() {
    return sessionStorage.getItem('egovp_project_name');
  }
  ...
  get name(){
    return sessionStorage.getItem('egovp_user_name');
  }

  get egovp_id(){
    return sessionStorage.getItem('egovp_user_id');
  }
}
