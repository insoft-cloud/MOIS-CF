...
export class OrgTempComponent implements OnInit, AfterViewChecked {
        ...
        spaceRole.some(model => {
          if(model.entity.username===username){
            model.entity.space_roles.filter(space_role => {
              switch (space_role){
                case 'space_manager' : $("#modal10_" + index + "_" +spaceid).prop('checked', true); break;
                case 'space_developer' : $("#modal11_" + index+ "_" +spaceid).prop('checked', true); break;
                case 'space_auditor' : $("#modal12_" + index+ "_" +spaceid).prop('checked', true); break;
              }});
            return true;
          }});});
  ...
  createSpace() {
    ...
    }, error1 => {this.common.isLoading = false;},() => {
      this.common.isLoading = false;
    });
  }
  ...
  userInvite() {
    ...
    //   this.common.doPost('/commonapi/v3/email/inviteOrg', params, '').subscribe(data => {
    //     if (data) {
