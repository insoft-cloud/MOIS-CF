...
export class SecurityService {
  ...
  doAuthorization() {
    ...
    //
    // let param = {
    //   userName : window.sessionStorage.getItem('egovp_user_name'),
    //   projectId: '',//window.sessionStorage.getItem('egovp_project_id'),
    //   clientId : appConfig['clientId'],
    //   clientSecret : appConfig['clientSecret']
    // };
    // this.common.doPost('/portalapi/v3/user/keycloak',param,null).subscribe(data => {
    //   console.log(data);
  ...
  doUserInfoProvider(userId: string) {
    ...
    const headers = new HttpHeaders()
      .append('Content-Type', 'application/x-www-form-urlencoded');
    return this.common.doGet('/commonapi/v2/user/' + userId, this.common.getToken()).retryWhen(error => {
      return error.flatMap((error: any) => {
        return Observable.of(error.status).delay(1000);
      }).take(3).concat(Observable.throw({error: 'Sorry, there was an error (after 3 retries)'}));
    }).subscribe(data => {
      if (data['User'] != null) {
        ...
        const nextUrl = sessionStorage.getItem('returnUrl');
  ...
  saveUserDB(userId: string) {
    ...
    this.common.doGet('/commonapi/v2/user/' + userId + '/uaa', this.common.getToken()).subscribe(data => {
      ...
      this.common.doPost('/commonapi/v2/user', params, this.common.getToken()).retryWhen(error => {
        return error.flatMap((error: any) => {
          return Observable.of(error.status).delay(1000);
        }).take(3).concat(Observable.throw({error: 'Sorry, there was an error (after 3 retries)'}));
      }).subscribe(data2 => {
