...
export class IndexCommonService {
  ...
  sendCreateEmail(url, authorization, email: string) {
    ...
    this.isUsed = false;
    let param = {userid: email, username: email};
    this.commonService.doPost(url + '/commonapi/v3/users/create/email', param, '').subscribe(data => {
      if (data['result'] === true) {
  ...
  sendResetEmail(url, authorization, email: string) {
    ...
    this.commonService.doPost(url + '/commonapi/v3/users/password/email', param, '').subscribe(data => {
      if (data['result'] === true) {
        this.isSendEmail = true;
      } else {
        this.isSendEmail = false;
