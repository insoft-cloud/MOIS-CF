...
export class AppMainComponent implements OnInit, OnDestroy{
  ...
  getAppStats(guid: string) {
    this.appMainService.getAppStats(guid).subscribe(data => {
      if (data) {
        ...
        var disk = 0;
        var cnt = 0;
        let maxmem = this.appSummaryMemory;
        let maxdisk = this.appSummaryDisk;
        $.each(data.instances, function (key, dataobj) {
          if (dataobj.stats != null) {
            ...
            if (!(null == dataobj.stats.usage.mem || '' == dataobj.stats.usage.mem)) mem = mem + (dataobj.stats.usage.mem) / (maxmem * 1024 * 1024 ) * 100;
            if (!(null == dataobj.stats.usage.disk || '' == dataobj.stats.usage.disk)) disk = disk + (dataobj.stats.usage.disk) / (maxdisk * 1024 * 1024 ) * 100;
            cnt++;
          }
        });
        if (cpu > 0) {
  ...
  getAppEnv(guid: any) {
    this.appMainService.getAppEnv(guid).subscribe(data => {
      ...
      if (JSON.stringify(data.system_env_json) != "{}") {
        if(!isNullOrUndefined(data.system_env_json.VCAP_SERVICES)){
  ...
  getCpuChart() {
    ...
    if(!isNullOrUndefined(this.cpu_Chart)){
      this.cpu_Chart.destroy();
    }
