        ...
        success: function(data) {
            ...
            if (data) {
                if(data.result == false) {
                    notifyAlert('error', '', data.data);
                    return false;
                }

                callback(data.data, param);
...
function getInitMarketPlaceURL() {
    ...
    procCallAjax("/v2/codedetail", "GET", param, procCallbackGetCodeMarketDetailList, $('#serviceModalLoadingBar'));
}

function procCallbackGetCodeMarketDetailList(data) {
    var market_html = "";
