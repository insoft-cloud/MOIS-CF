...
public class CommonService {
     ...
     * @return map map
     */
    public <T> T procCommonApiRestTemplate2(String reqUrl, HttpMethod httpMethod, Object obj, String reqToken, Class<T> resClazz) {
        restTemplate = new RestTemplate();
        HttpHeaders reqHeaders = new HttpHeaders();
        reqHeaders.add(AUTHORIZATION_HEADER_KEY, base64Authorization);
        if (null != reqToken && !"".equals(reqToken)) reqHeaders.add(CF_AUTHORIZATION_HEADER_KEY, reqToken);
        ...
        LOGGER.info("CommonApiUrl::"+commonApiUrl + reqUrl);
        HttpEntity<Object> reqEntity = new HttpEntity<>(obj, reqHeaders);
        ResponseEntity<T> resEntity = restTemplate.exchange(commonApiUrl + reqUrl, httpMethod, reqEntity, resClazz);
     ...
     * @return map map
     */
    public <T> T procUAARestTemplate(String reqUrl, HttpMethod httpMethod, Object bodyObject, String reqToken, Class<T> resClazz) {
        restTemplate = new RestTemplate();
        HttpHeaders reqHeaders = new HttpHeaders();
        reqHeaders.add(AUTHORIZATION_HEADER_KEY, reqToken);
        reqHeaders.add("Content-Type", "application/json");

        HttpEntity<Object> reqEntity = new HttpEntity<>(bodyObject, reqHeaders);

        ResponseEntity<T> resEntity = restTemplate.exchange(uaaTarget + reqUrl, httpMethod, reqEntity, resClazz);
        LOGGER.info("procRestStorageApiTemplate reqUrl :: {} || resultEntity type :: {}", uaaTarget + reqUrl, resEntity.getHeaders().getContentType());
        LOGGER.info("procRestStorageApiTemplate response Http status code :: {}", resEntity.getStatusCode());
        return resEntity.getBody();
    }
