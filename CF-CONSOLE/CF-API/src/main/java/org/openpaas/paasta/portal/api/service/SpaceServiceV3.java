...
public class SpaceServiceV3 extends Common {
     ...
     * @throws Exception the exception
     */
    public GetSpaceSummaryResponse getAdminSpaceSummary(String spaceId) throws Exception {
    ...
    public CreateSpaceResponse createSpace2(String name, String organizationId, String usrid, String token) {
        final CreateSpaceResponse response = cloudFoundryClient(connectionContext(), tokenProvider(token)).spaces().create(CreateSpaceRequest.builder().name(name).organizationId(organizationId).build()).block();

            associateSpaceManager(response.getMetadata().getId(),usrid);
            associateSpaceDeveloper(response.getMetadata().getId(), usrid);
            associateSpaceAuditor(response.getMetadata().getId(), usrid);
