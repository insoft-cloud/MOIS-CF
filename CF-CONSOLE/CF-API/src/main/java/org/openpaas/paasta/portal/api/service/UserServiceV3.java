...
import org.openpaas.paasta.portal.api.model.MoisUser;
...
public class UserServiceV3 extends Common {
    ...
    public UserServiceV3(RestTemplate restTemplate, LoginServiceV1 loginServiceV1, VaultTemplate vaultTemplate) {
    ...
    public Map KeycloakUser(MoisUser moisUser) throws Exception {
        LOGGER.info(moisUser.toString());
        LOGGER.info(PATH_PROJECT + "user/" + moisUser.getUserName());
        VaultResponseSupport<Object> user_result = vaultTemplate.read(PATH_PROJECT + "user/" + moisUser.getUserName(), Object.class);
        Map<String, Map<String, String>> u_result  = (Map<String, Map<String, String>>) user_result.getData();
        StringBuffer user_id = new StringBuffer().append(u_result.get("data").get(Constants.USER_ID));
        StringBuffer pwd = new StringBuffer().append(u_result.get("data").get(Constants.USER_PWD));
        CloudCredentials cc = new CloudCredentials(moisUser.getUserName(), pwd.toString());
        Map map = new HashMap();
        map.put("tokenData", new CloudFoundryClient(cc, new URI(apiTarget).toURL(), true).login());
        map.put("userID",user_id.toString());
        return map;
    }

    public Map CF_PW(MoisUser moisUser) throws Exception {
        VaultResponseSupport<Object> user_result = vaultTemplate.read(PATH_PROJECT + "user/" + moisUser.getUserName(), Object.class);
        Map<String, Map<String, String>> u_result  = (Map<String, Map<String, String>>) user_result.getData();
        StringBuffer pwd = new StringBuffer().append(u_result.get("data").get(Constants.USER_PWD));
        Map map = new HashMap();
        map.put("Password",pwd.toString());
        return map;
    }
