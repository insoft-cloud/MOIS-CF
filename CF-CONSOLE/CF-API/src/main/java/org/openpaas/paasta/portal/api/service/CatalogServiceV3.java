...
import org.openpaas.paasta.portal.api.model.ServiceInstanceProvisionRequest;
...
public class CatalogServiceV3 extends Common {
    ...
    public Map<String, Object> createApp(Catalog param, String token, String token2, HttpServletResponse response) {
        ...
        try {
            ...
            LOGGER.info("file ::" + file.getName() + ":::" + file.length());
    ...
    private void fileUpload(File file, String applicationid, ReactorCloudFoundryClient reactorCloudFoundryClient) throws Exception {
        ...
        while(i <= 50) {
            i++;
            LOGGER.info("FileUpload :: Step " + i);
            try {
                Thread.sleep(200);
                if (file.getName().substring(file.getName().length() - 3).equals("tgz")) {
                    reactorCloudFoundryClient.applicationsV2().uploadDroplet(UploadApplicationDropletRequest
                            .builder().applicationId(applicationid).droplet(file.toPath()).build()).block();
                } else {
                    reactorCloudFoundryClient.
    ...
    private File createTempFile(Catalog param, String token2, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            is = (new URL(param.getAppSampleFilePath()).openConnection()).getInputStream();
    ...
    public CreateServiceInstanceResponse CreateServcieInstance(ServiceInstanceProvisionRequest param,ReactorCloudFoundryClient reactorCloudFoundryClient){
        return  reactorCloudFoundryClient.serviceInstances()
                .create(CreateServiceInstanceRequest.builder()
                        .name(param.getContext().getProperty("instance_name").toString())
                        .spaceId(param.getContext().getProperty("space_guid").toString())
                        .parameters(param.getParameters())
                        .servicePlanId(param.getPlan_id())
                        .build()).block();
    }
    ...
    public UpdateServiceInstanceResponse updateServicePlan(String instance_id, ServiceInstanceProvisionRequest param, ReactorCloudFoundryClient reactorCloudFoundryClient){
        return reactorCloudFoundryClient.serviceInstances().update(
                UpdateServiceInstanceRequest.builder()
                .name(param.getContext().getProperty("instance_name").toString())
                .putAllParameters(param.getParameters())
                .servicePlanId(param.getPlan_id())
