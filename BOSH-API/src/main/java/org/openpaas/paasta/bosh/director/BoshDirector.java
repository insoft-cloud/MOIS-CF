...
import org.openpaas.paasta.bosh.model.cloudconfig.*;
import org.openpaas.paasta.bosh.model.cpiconfig.*;
...
public class BoshDirector extends BoshCode {
    ...
    public List<Map> getDeploymentTasks(String deployment) throws Exception {
        return resEntityList("/tasks?limit=1&deployment="+deployment, HttpMethod.GET, ContentsType.TextYaml, null);
    }
    ...
    //Get Cloud Config
    public List<Map> getCloud_Config() throws Exception{
        return resEntityList("/configs?type=cloud&name=default", HttpMethod.GET, ContentsType.TextYaml, null);
    }
    //Get Cloud Config
    public List<Map>  getCloud_Config_Cpi() throws Exception{
        return resEntityList("/configs?type=cpi", HttpMethod.GET, ContentsType.TextYaml, null);
    }
    ...
    //Get Cloud Config Model
    public String update_Cloud_Config(String manifest) throws Exception{
        //String param = "{\"name\": \"default\", \"type\": \"cloud\", \"content\": \"--- {}\"}";
        return resEntityS("/configs", HttpMethod.POST, ContentsType.ApplicationJson, manifest);
    }

    //Diff config
    public Map postDiffConfig(String name, String type, String content) throws Exception {
        String param = "{\"name\": \"" + name + "\", \"type\": \"" + type + "\", \"content\": \"" + content + "\"}";
        return resEntityM("/configs/diff", HttpMethod.POST, ContentsType.TextYaml, param);
    }
    ...
    public boolean deploy(String deployment_name, String service_name) throws Exception {
        ...
        try {
             ...
             * 배포
             */
            System.out.println(postCreateAndUpdateDeployment(manifest));
    ...
    public String updateServiceInstance(String deployment_name, String job_name, String vm_type, String persistent_disk) throws Exception {
        ...
        for (Map map : instance_groups) {
            if (map.get("name").equals(job_name)) {
                ...
                map.remove("persistent_disk");
                map.put("persistent_disk_type", persistent_disk);
            }
        }
    ...
    public String createServiceInstance(String job_name, String vm_type, String yml, String persistent_disk, Map<?,?> properties) throws Exception {
        DumperOptions options = new DumperOptions();
        options.setDefaultFlowStyle(DumperOptions.FlowStyle.BLOCK);
        Yaml loader = new Yaml(options);
        ...
        ((Map)manifest_map.get("properties")).forEach( (key, value) ->{
            if(!properties.containsKey(key)){
                try {
                    throw new Exception("파라미터값이 일치하지 않습니다.");
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
        } );
        manifest_map.put("properties", properties);
    ...
    public boolean Update_Cloud_Config(BoshAzs boshAzs, BoshNetwork network) throws Exception {

        try {
            Map manifest = this.getCloud_Config().get(0);
            String contents = manifest.get("content").toString();
            DumperOptions options = new DumperOptions();
            options.setDefaultFlowStyle(DumperOptions.FlowStyle.AUTO);
            Yaml loader = new Yaml(options);
            Map manifest_map = loader.load(contents);
            ObjectMapper mapper = new ObjectMapper();
            mapper.configure(DeserializationFeature.ACCEPT_SINGLE_VALUE_AS_ARRAY, true);
            BoshCloud instance_groups = mapper.convertValue(manifest_map,BoshCloud.class);
            instance_groups.getNetworks().add(network);
            instance_groups.getAzs().add(boshAzs);
            mapper.setSerializationInclusion(JsonInclude.Include.NON_NULL);
            manifest.put("content", loader.dump(mapper.convertValue(instance_groups, Map.class)));
            manifest.remove("id");
            manifest.remove("team");
            manifest.remove("created_at");
            manifest.remove("current");
            update_Cloud_Config(mapper.writeValueAsString(manifest));

            //manifest_map.get("networks");

            // BoshNetwork networks = (BoshNetwork)manifest_map.get("networks");

            /*
             * Manifest 값의 특정 인스턴스 값을 변환
             */
            //networks = manifestParser(networks, service_name);


            /*
             * 배포
             */
            //postCreateAndUpdateDeployment(networks);
    ...
    public boolean Update_Cloud_Config_CPI(BoshCpi cpi) throws Exception {
        DumperOptions options = new DumperOptions();
        options.setDefaultFlowStyle(DumperOptions.FlowStyle.AUTO);
        Yaml loader = new Yaml(options);
        ObjectMapper mapper = new ObjectMapper();
        ...
        mapper.setSerializationInclusion(JsonInclude.Include.NON_NULL);
        try {
            if(getCloud_Config_Cpi().size() != 0){
            Map content_cpi = this.getCloud_Config_Cpi().get(0);
            String contents = content_cpi.get("content").toString();
            Map manifest_map = loader.load(contents);
            BoshCpis instance_groups = mapper.convertValue(manifest_map, BoshCpis.class);
            instance_groups.getCpis().add(cpi);
            content_cpi.put("content", loader.dump(mapper.convertValue(instance_groups, Map.class)));
            content_cpi.remove("id");
            content_cpi.remove("team");
            content_cpi.remove("created_at");
            content_cpi.remove("current");
            update_Cloud_Config(mapper.writeValueAsString(content_cpi));
            } else {
                BoshCpis boshCpis = new BoshCpis();
                List<BoshCpi> boshCpis1 = new ArrayList<>();
                boshCpis1.add(cpi);
                boshCpis.setCpis(boshCpis1);
                Map content_cpi = new HashMap();
                content_cpi.put("content", loader.dump(mapper.convertValue(boshCpis, Map.class)));
                content_cpi.put("name", "default");
                content_cpi.put("type", "cpi");
                update_Cloud_Config(mapper.writeValueAsString(content_cpi));
            }
            //manifest_map.get("networks");
